...
if __name__ == "__main__":
    response = GPT_trans("由于高光谱图像仅包含有限数量的物质，因此可以将其视为嵌入高维空间的一系列低维子流形。 通常来说位于同一子流形中的像素有更高的相似性，而不同子流形之间的像素相似度更低。通过对高空间分辨率的多光谱图像施加流形正则化可以有效提升重构图像的空间分辨率。对多光谱图像进行空间聚类可以得到K个子流形A和每个子流形对应的聚类中心B。通过构造不同的图拉普拉斯矩阵可以将多光谱图像的空间特征嵌入到重构图像中，提升重构图像的空间细节")
